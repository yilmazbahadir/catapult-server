...
#define TEST_CLASS FinalizationServiceTests
	...
	namespace {
		...
		struct FinalizationServiceTraits {
			...
			static auto GetWriters(const extensions::ServiceLocator& locator) {
				return locator.service<net::PacketWriters>("finalization");
			}
		};
	...
	TEST(TEST_CLASS, ConnectPeersTaskIsScheduled) {
		test::AssertRegisteredTask(TestContext(), Num_Expected_Tasks, "connect peers task for service Finalization");
	}

	TEST(TEST_CLASS, PullPtTaskIsScheduled) {
		test::AssertRegisteredTask(TestContext(), Num_Expected_Tasks, "pull finalization messages task");
	}
