...
#define FIELD(X) expectedSize += sizeof(VoteProof::X);
		VOTE_FIELDS
#undef FIELD
...
#undef VOTE_FIELDS
	...
	namespace {
		...
		std::unique_ptr<PackedFinalizationProof> CreateProof(uint32_t numVotes) {
			uint32_t proofSize = sizeof(PackedFinalizationProof) + numVotes * sizeof(VoteProof);
...
#define PROOF_FIELDS FIELD(VoteProofsCount) FIELD(FinalizedHash) FIELD(FinalizedHeight) FIELD(StepIdentifier)
	...
	TEST(TEST_CLASS, PackedFinalizationProofHasExpectedSize) {
		// Arrange:
		auto expectedSize = sizeof(TrailingVariableDataLayout<PackedFinalizationProof, VoteProof>);

#define FIELD(X) expectedSize += sizeof(PackedFinalizationProof::X);
		PROOF_FIELDS
#undef FIELD
